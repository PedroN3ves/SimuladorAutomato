...
from pilha import AutomatoPilha, EPSILON
...
class PilhaGUI:
    ...
    def _build_bottom_bar(self):
        ...
        ttk.Label(bottom, text="Entrada:").pack(side=tk.LEFT)
    ...
    def cmd_step(self):
        ...
        if self.sim_step < len(self.history) - 1:
            ...
            _, accepted = self.automato.simulate_history(self.input_entry.get()) # Re-simula para obter o resultado final correto
            result = "ACEITA" if accepted else "REJEITADA"
            messagebox.showinfo("Fim da Simulação", f"A cadeia foi {result}.", parent=self.root)
    ...
    def on_canvas_click(self, event):
        ...
        if self.mode == "add_state":
            ...
            self._set_mode("select") # Garante que o modo volte ao normal mesmo se o usuário cancelar ou errar
    ...
    def _edit_edge(self, src: str, dst: str):
        ...
        for (s, inp, pop), transitions in self.automato.transitions.items():
            if s == src:
                for (d, push) in transitions:
                    if d == dst:
                        current_labels.append(f"{inp or '&'}, {pop or '&'} / {push or '&'}")
        ...
        if new_labels_str is not None:
            # Remove todas as transições antigas entre src e dst
            # (Esta é uma simplificação. Uma implementação mais robusta seria mais seletiva)
            # Adiciona as novas transições
            for label in new_labels_str.split('\n'):
                if '/' in label:
                    read_part, push_part = label.split('/', 1)
                    input_sym, pop_sym = (read_part.split(',') + [EPSILON])[:2]
    ...
    def _draw_edges_and_states(self):
        ...
        for (src, inp, pop), transitions in self.automato.transitions.items():
            for (dst, push) in transitions:
                label = f"{inp or 'ε'}, {pop or 'ε'} / {push or 'ε'}"
