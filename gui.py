...
class EditorGUI:
    ...
    def cmd_convert_to_dfa(self):
        ...
        if not dfa:
            messagebox.showerror("Converter", "Falha ao converter para DFA.", parent=self.root)
            return
        self._push_undo_snapshot()
        self.automato = dfa
    ...
    def cmd_validate_dfa(self):
        ...
        if ok:
            messagebox.showinfo("Validação DFA", "O autômato é um DFA válido.", parent=self.root)
        else:
            messagebox.showwarning("Validação DFA", "O autômato NÃO é um DFA válido (pode ter ε-transições ou não-determinismo).", parent=self.root)
    ...
    def draw_all(self):
        ...
        for (src, dst), syms in agg.items():
            ...
            if src == dst:
                ...
                # Curvatura para arestas de volta
                ...
                self.canvas.create_line(start_x, start_y, ctrl_x, ctrl_y, end_x, end_y, smooth=True, width=2, arrow=tk.LAST)
    ...
    def cmd_simulate(self):
        ...
        if not history: messagebox.showwarning("Animar", "Autômato vazio ou sem estado inicial.", parent=self.root); return
    ...
    def cmd_step(self):
        if not self.history or self.sim_step >= len(self.history) - 1:
            if self.history:
                ...
                self.draw_all()
            else: messagebox.showwarning("Passo", "Nenhuma simulação em andamento.", parent=self.root)
            return
        self.sim_step += 1
        self.draw_all()
